...
import { createTest } from '../aws-ecs/legacy/create-test.ts';
import * as util from '../aws-ecs/legacy/util.ts';
import { QueueConsumer } from './aqs-queue-consumer.ts';
...
import { IMAGE_VERSION, WAIT_TIMEOUT } from '../aws-ecs/legacy/constants.ts';
import { sleep, Timeout } from '../aws-ecs/legacy/time.ts';
...
class PlatformAzureACI {
  ...
  constructor(script, variablePayload, opts, platformOpts) {
    ...
    if (!regionNames.includes(this.region)) {
      const err: any = new Error(`Invalid region: ${this.region}`);
    ...
    if (
      ...
      const err: any = new Error('Azure credentials not found');
    ...
    if (
      ...
      const err: any = new Error('Azure configuration not found');
...
`);
    ...
    const { manifest }: any = await createTest(this.opts.absoluteScriptPath, {
      name: this.testRunId,
