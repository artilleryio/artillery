...
test('reports in console and json report correctly', async (t) => {
  ...
  assert.ok(output.stdout.includes('plugins.metrics-by-endpoint./dino.codes.200:'));
  assert.ok(output.stdout.includes('plugins.metrics-by-endpoint./pony.codes.200:'));
  assert.ok(output.stdout.includes('plugins.metrics-by-endpoint.response_time./dino'));
  assert.ok(output.stdout.includes('plugins.metrics-by-endpoint.response_time./pony'));

  //Assert json report includes plugin metrics
  const jsonReport = require(reportPath);

  assert.strictEqual(jsonReport.aggregate.counters[
      'plugins.metrics-by-endpoint./dino.codes.200'
    ], 4);
  assert.strictEqual(jsonReport.aggregate.counters[
      'plugins.metrics-by-endpoint./pony.codes.200'
    ], 4);
  assert.ok(Object.keys(jsonReport.aggregate.summaries).includes(
...
test("Reports correctly when 'parallel' is used", async (t) => {
  ...
  for (const path of requestPaths) {
    assert.strictEqual(report.aggregate.counters[
      ...
      ], expectedVus, `${expectedVus} requests to ${path} should have returned 200`);
  }
});
...
test('Reports correctly when `groupDynamicURLs` is set to true (default)', async (t) => {
  ...
  assert.strictEqual(report.aggregate.counters[
    ...
    ], 4, 'should have counter metrics including templated url and no query strings');
  assert.strictEqual(report.aggregate.counters[
      'plugins.metrics-by-endpoint./dino/{{ $randomString() }} (GET /dino).codes.200'
    ], 4, 'should have counter metrics including templated url with request name specified');
  assert.strictEqual(report.aggregate.counters['plugins.metrics-by-endpoint./pony.codes.200'], 4, 'should display counter metrics for /pony as normal');

  assert.ok(Object.keys(report.aggregate.summaries).includes(
...
test('Reports correctly when `groupDynamicURLs` is explicitly set to false', async (t) => {
  ...
  assert.ok(countersWithName.length > 0, `should have counter metrics without the templated url, got ${countersWithName}`);
  assert.ok(countersWithoutName.length > 0, `should have counter metrics without the templated url and request name specified, got ${countersWithoutName}`);
  assert.ok(regularPonyCounter.length === 1, `should display counter metrics for /pony as normal, got ${regularPonyCounter}`);
});
