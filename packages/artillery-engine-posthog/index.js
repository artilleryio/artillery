...
function PosthogEngine(script, ee, helpers) {
  this.script = script;
  this.ee = ee;
...
PosthogEngine.prototype.createScenario = function createScenario(scenarioSpec, ee) {
  const tasks = scenarioSpec.flow.map(rs => this.step(rs, ee));

  return this.compile(tasks, scenarioSpec.flow, ee);
};

PosthogEngine.prototype.step = function step (rs, ee) {
  const self = this;
...
PosthogEngine.prototype.compile = function compile (tasks, scenarioSpec, ee) {
  const self = this;
