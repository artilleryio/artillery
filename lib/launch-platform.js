...
class Launcher {
  ...
  async flushIntermediateMetrics(flushAll = true) {
    ...
    const earliestPeriodAvailable = Object.keys(this.metricsByPeriod).filter(x => this.periodsReportedFor.indexOf(x) === -1).sort()[0];
