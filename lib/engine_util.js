...
function approx(sApprox) {
	if (sApprox == undefined) return sApprox;
	if (! sApprox.indexOf) return sApprox;
	if (sApprox.indexOf(":") < 0) return sApprox;

	let inputs = sApprox.split(":");
	let nb = parseInt(inputs[0]);
	let approxPercent = parseInt(inputs[1]);
	...
	return nb - approx + Math.random() * 2 * approx;
}

function createThink(requestSpec) {
  let thinktime = approx(requestSpec.think) * 1000;
