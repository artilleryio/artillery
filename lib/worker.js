...
process.env.WORKER_ID = threadId;
...
artillery.globalEvents.on('log', (msg, opts) => {
  send({event: 'log', msg, opts });
});
...
async function prepare(opts) {
  ...
  if (script.config.processor) {
    let processorPath = path.resolve(path.dirname(absoluteScriptPath), script.config.processor);
  ...
  // launch-local instead. (If we subscribe plugins to events here,
...
//         });
        ...
        if (result.version === 1) {
          ...
          result.plugin = new result.PluginExport.Plugin(script, stubEE, options);
          global.artillery.plugins.push(result);
        } else {
          // TODO:
        }
  ...
  // TODO: use await
  createRunner(script, payload, options).then(function(runner) {
