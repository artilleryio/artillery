...
async function addOverrides(script, flags) {
  ...
  const result = _.mergeWith(script, o, function customizer(objVal, srcVal, k, obj, src, stack) {
    if (_.isArray(srcVal)) {
      return srcVal;
    }
    else {
...
async function checkConfig(script, scriptPath, flags) {
  ...
  if (flags.environment) {
    ...
    if (script.config.environments && script.config.environments[flags.environment]) {
...
function padded(str1, str2, length = 60, formatPadding = chalk.gray) {
  const truncated = maybeTruncate(str1, length);
  return truncated + ' ' + formatPadding('.'.repeat(length - truncated.length)) + ' ' + str2;
}
