...
//                                'warn');
  ...
  async runLocal(hook, script, payload, options) {
    if (!script[hook]) {
      return {};
    }

    const runnableScript = prepareScript(script, payload);
    const context = await handleScriptHook(hook, runnableScript, this.events);

    debug(`hook ${hook} context`, context);
  ...
  async run() {
    ...
    const context = await this.runLocal('before', this.script, this.payload, this.opts)
    ...
    for(const script of this.workerScripts) {
      ...
      w1.events.on('done', async (message) => {
        ...
        if(Object.keys(this.finalReportsByWorker).length === this.count) {
          ...
          // 'after' hook is executed in the main thread, when all the workers have completed
          // their jobs - TODO pass context from workers
          await this.runLocal('after', this.script, this.payload, this.opts)
    ...
    // the initial context is stringified and passed to the workers
    const contextString = JSON.stringify(context);

    for (const [workerId, w] of Object.entries(this.workers)) {
      runAll.push(w.proc.run(contextString));
    }
