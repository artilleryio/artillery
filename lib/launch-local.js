...
//                                'warn');
  ...
  async runLocal(hook, script, payload, initialContextVars) {
    if (!script[hook]) {
      return {};
    }
  ...
  async run() {
    ...
    const contextVars = await this.runLocal(
      'before',
      this.script,
      this.payload
      // this.opts
    ...
    for (const script of this.workerScripts) {
      ...
      w1.events.on('done', async (message) => {
        ...
        if (Object.keys(this.finalReportsByWorker).length === this.count) {
          ...
          // (scenarioContext || {}).vars || contextVars
          await this.runLocal('after', this.script, this.payload, contextVars);
