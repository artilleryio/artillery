...
async function handler(event, context) {
  ...
  try {
    ...
    if (err || code !== 0) {
      console.log(err);
      await mq.send({ event: 'workerError', reason: 'ArtilleryError', logs: { stdout, stderr } });
    }
  } catch (err) {
    console.error(err);
...
async function execArtillery(options) {;
  ...
  return runProcess(NODE_BINARY_PATH || '/var/lang/bin/node', [ARTILLERY_BINARY_PATH||'./node_modules/artillery/bin/run'].concat(ARTILLERY_ARGS), { env, log: true });
}
