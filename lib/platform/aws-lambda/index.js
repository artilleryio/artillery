...
class PlatformLambda {
  ...
  async init() {
    ...
    fs.copyFileSync(path.resolve(a9basepath, 'package-lock.json'), path.join(dirname, 'node_modules', 'artillery', 'package-lock.json'));

    const { stdout2, stderr2, status2, error2 } = spawnSync('npm', ['ci', '--omit', 'dev'], {cwd: path.join(dirname, 'node_modules', 'artillery')});
    if (error2) {
    ...
    global.artillery.ext({
      ...
      method: async (event) => {
        ...
        if (event.flags && event.flags.platform === 'aws:lambda') {
          let price = 0;
          // TODO: ARM if running on Graviton
          if (!prices[self.region]) {
            price = prices.base.x86;
          } else {
            price = prices[self.region].x86;
          }
          ...
          console.log(`\nEstimated AWS Lambda cost: $${cost}\n`);
        }
      }
    });
  }
