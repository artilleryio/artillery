...
const createRunner = require('../launch-local');
...
class RunCommand extends Command {
  ...
  async run() {
    ...
    try {
      ...
      let runner = await createRunner(script, script.config.payload, runnerOpts);
      let intermediates = [];
      ...
      if (process.env.CUSTOM_REPORTERS) {
        ...
        customReporterNames.forEach(function(name) {
          ...
          const reporter = createReporter(runner.events, flags);
      ...
      runner.events.on('done', async function(stats) {
        let report;
        if(artillery.runtimeOptions.legacyReporting) {
...
async function sendTelemetry(script, flags) {
  ...
  try {
    ...
    if(script.config && script.config.target) {
      const hash = crypto.createHash('sha1').update(script.config.target).digest('base64');
      properties.targetHash = hash;
    }
    ...
    if(script.config.plugins) {
      ...
      const OFFICIAL_PLUGINS = ['expect', 'publish-metrics', 'metrics-by-endpoint', 'hls', 'fuzzer', 'ensure'];
...
function getLogFilename(output, nameFormat) {
  ...
  if (!isDir && output) {
    ...
    logfile = path.join(
      output,
      moment().format(nameFormat || defaultFormat)
    );
  }
