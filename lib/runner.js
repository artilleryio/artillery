...
function runScenario(script) {
  ...
  var scenario = Workers[spec.engine || 'http'].create(
    spec, script.config, initialContext);

  Stats.generatedScenarios.inc();
  PeriodicStats.generatedScenarios.inc();

  scenario.on('scenarioStarted', function() {
    pendingScenarios.inc();
  });
  scenario.on('scenarioCompleted', function() {
