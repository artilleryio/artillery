...
Runner.prototype.run = function run() {
  ...
  L.each(workerScripts, (script) => {
    ...
    workerProcess.send({
      ...
      opts: {
        script: script,
        payload: this._payload, // This is going to be inefficient with large payload files
        options: this._opts
      }
    });
  });

  setInterval(this._sendStats.bind(this), this._script.config.statsInterval * 1000).unref();
  return this;
};

Runner.prototype._sendStats = function() {
  this.events.emit('stats', stats.combine(this._intermediates).report());
  this._intermediates = [];
};
...
Runner.prototype._onWorkerMessage = function _onWorkerMessage(message) {
  ...
  if (message.event === 'done') {
    ...
    if (this._activeWorkerCount() === 0) {
      this._sendStats();
      let report = stats.combine(this._allIntermediates).report();
      delete report.concurrency;
      this.events.emit('done', report);
    }
  }
};
