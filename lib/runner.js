...
function run(script, ee, options) {
  ...
  var phases = _.map(script.config.phases, function(phaseSpec, i) {
    ...
    var task = function(callback) {
      ...
      ee.emit('phaseStarted', {index: i, name: phaseSpec.name, duration: phaseSpec.duration});
      var ar = 1000 / (phaseSpec.users / phaseSpec.duration);
      var pp = PoissonProcess.create(ar, function() {
      ...
      setTimeout(function() {
        ...
        ee.emit('phaseCompleted', {index: i, name: phaseSpec.name, duration: phaseSpec.duration});
        pp.stop();
