...
function ConsoleReporter(opts) {
  opts = opts || {};
  this.outputFormat = opts.outputFormat || process.env.OUTPUT_FORMAT || 'old';

  this.quiet = opts.quiet;
  this.spinner = ora({
    // TODO: customize via config.console-reporter or some such
    spinner: 'line'
    // {
    //   interval: 100,
    //   frames: ['', '▹', '▹▹', '▹▹▹', '▹▹▹▹', '▹▹▹▹▹▹', '▹▹▹▹▹▹', '']
...
ConsoleReporter.prototype.printReport = function printReport(report, opts) {
  ...
  if (this.outputFormat === 'old') {
    ...
    console.log('  Avg responses/sec: %s', report.rps.mean);
    ...
    if (_.size(report.customStats) > 0 || _.size(report.counters) > 0) {
      _.each(report.customStats, function(r, n) {
        if (n.startsWith('engine.') || n.startsWith('scenarios.')) {
          return;
        }
        console.log('%s:', n);
    ...
    _.each(report.counters, function(value, name) {
        if (name.startsWith('engine.') || name.startsWith('scenarios.') || name.startsWith('errors.')) {
          return;
        }
  ...
  if (this.outputFormat === 'table') {
    ...
    if (_.size(report.customStats) > 0 || _.size(report.counters) > 0) {
      ...
      _.sortBy(Object.keys(report.customStats)).forEach((n) => {
        const r = report.customStats[n];
        t.push([`${n}.min`, r.min]);
        t.push([`${n}.max`, r.max]);
        t.push([`${n}.median`, r.median]);
        t.push([`${n}.p95`, r.p95]);
        t.push([`${n}.p99`, r.p99]);
      });
      _.sortBy(Object.keys(report.counters)).forEach((name) => {
