...
ConsoleReporter.prototype.printReport = function printReport(report, opts) {
  ...
  if (this.outputFormat === 'new') {
    ...
    // TODO: Display codes, errors etc in a sub-list -- with a simple heuristic:
    // 1. Remove core, engine, plugins, or user
    // 2. Are there metric names that share a prefix? If so, display them in a sublist
    //    OR: allow for those prefixes to be annotated by the user of the interface
    // user.my_custom_metrics.one
    // user.my_custom_metrics.two
    // user.my_custom_metrics.three
    // vs
    // user.my_custom_metrics.one.something
    // user.my_custom_metrics.one.something_else
    //
    // So - use top level if you don't want them displayed in a hierarchy?
    // Always display in a hierarchy?
