...
ConsoleReporter.prototype.stats = function stats(data) {
  ...
  this.printReport(report, {lastTarget: data.lastTarget});
  this.toggleSpinner();
};
...
ConsoleReporter.prototype.done = function done(data, lastTarget) {
  if (this.quiet) {
    return this;
  }
  const report = data.report();
...
ConsoleReporter.prototype.printReport = function printReport(report, opts) {
  ...
  if (opts.lastTarget) {
    console.log('  Last Target:         %s', opts.lastTarget);
  }
