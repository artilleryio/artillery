...
const SSMS = require('../core/lib/ssms').SSMS;
...
ConsoleReporter.prototype.printReport = function printReport(report, opts) {
  ...
  if (opts.printPeriod !== false) {
    ...
    if (typeof report.period !== 'undefined') {
      ...
      // artillery.log(util.padded('time_window:', timeWindowEnd));
    } else {
  ...
  if (this.outputFormat === 'new') {
    ...
    const coreRates = Object.keys(report.rates).filter((name) =>
      name.startsWith('core.')
    );
    const engineRates = Object.keys(report.rates).filter((name) =>
      name.startsWith('engine.')
    );
    const otherRates = Object.keys(report.rates).filter(
      (name) =>
        coreRates.indexOf(name) === -1 && engineRates.indexOf(name) === -1
    );

    report.counters = report.counters || {};
    const coreCounters = Object.keys(report.counters).filter((name) =>
      name.startsWith('core.')
    );
    const engineCounters = Object.keys(report.counters).filter((name) =>
      name.startsWith('engine.')
    );
    const otherCounters = Object.keys(report.counters).filter(
      (name) =>
        coreCounters.indexOf(name) === -1 && engineCounters.indexOf(name) === -1
    );

    report.summaries = report.summaries || {};
    const coreSummaries = Object.keys(report.summaries).filter((name) =>
      name.startsWith('core.')
    );
    const engineSummaries = Object.keys(report.summaries).filter((name) =>
      name.startsWith('engine.')
    );
    const otherSummaries = Object.keys(report.summaries).filter(
      (name) =>
        coreSummaries.indexOf(name) === -1 &&
        engineSummaries.indexOf(name) === -1
    );
...
function printRates(rates, report) {
  return rates.sort().map((name) => {
    return util.padded(`${name}:`, report.rates[name]) + '/sec';
  });
}
