  ...
  async.waterfall([
    function readScript(callback) {
      ...
      fs.readFile(scriptPath, 'utf-8', function(err, data) {
        ...
        if (err) {
          return callback(new Error('File not found ' + scriptPath), null);
        }
        ...
        try {
          ...
          return callback(new Error('File ' + scriptPath + ' does not appear to be valid JSON'), null);
        }
    ...
    function readPayload(context, callback) {
      ...
      if (options.payload) {
        ...
        csv(csvdata, function(err, payload) {
          ...
          if (err) {
            return callback(new Error('File ' + options.payload + ' does not appear to be valid CSV'), null);
          }
        ...
        if (context.script.config.payload) {
          console.log('WARNING: payload file not set, but payload is configured in %s\n', scriptPath);
        }
    ...
    function(err, result) {
      ...
      ee.on('stats', function(stats) {
        ...
        console.log();
        console.log('intermediate stats at: %s', new Date().toUTCString());
        console.log('new scenarios: %s', stats.generatedScenarios.toJSON());
        console.log('completed scenarios: %s', stats.completedScenarios.toJSON());
        console.log('completed requests: %s', stats.completedRequests.toJSON());
        console.log('errors: %s', stats.errors.toJSON());
      ...
      ee.once('done', function(stats) {
        ...
        var latency = stats.latencyNs.toJSON();
          ...
          _.each(result.script.config.ensure, function(max, k) {
            ...
            if (latency[bucket]) {
              if (latency[bucket] / 1e6 > max) {
                var message = util.format('ensure condition failed: ensure.%s < %s', bucket, max);
                if (tty.isatty(process.stdout)) {
