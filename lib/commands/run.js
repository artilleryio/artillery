...
function run(scriptPath, options) {
  ...
  async.waterfall([
    ...
    function(err, result) {
      ...
      //Check if profiler option has been passed
      if (options.profiler) {
        result.script.pid = Number(options.profiler);
      }
...
function humanize(report) {
  ...
  // console.log only when cpu and memory exist
  if (report.cpu) {
    console.log('  CPU:');
    console.log('    min: %s', report.cpu.min);
    console.log('    max: %s', report.cpu.max);
    console.log('    median: %s', report.cpu.median);
    console.log('    p95: %s', report.cpu.p95);
    console.log('    p99: %s', report.cpu.p99);
  }
  if (report.memory) {
    console.log('  Memory:');
    console.log('    min: %s', report.memory.min);
    console.log('    max: %s', report.memory.max);
    console.log('    median: %s', report.memory.median);
    console.log('    p95: %s', report.memory.p95);
    console.log('    p99: %s', report.memory.p99);
  }
