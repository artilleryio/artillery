            ...
            function done(err) {
              if (params.capture || params.match) {
                captureOrMatch(params, res, context, function(err, result) {
                  ...
                  //
                  // TODO: Fail the scenario if any strict matches failed.
                  //
...
// TODO: Multiple captures and matches - presume is an array in the code below.
//
// { "post": { "url": "/", "capture": [], "match": [] }
// match is like assert - i.e. if it doesn't match, abort by default - how do we report
// it to the user though?
function captureOrMatch(params, response, context, done) {
  ...
  let parser = getParserAndExtractor(params, response).parser;
  let extractor = getParserAndExtractor(params, response).extractor;
  ...
  parser(content, function(err, doc) {
    ...
    if (params.match) {
      ...
      let got = extractor(doc, expr);
      let expected = template(params.match.value, context);
      debug('match: %s, expected: %s, got: %s', expr, expected, got);
      if (result !== expected) {
        result.matches[expr] = {
          success: false,
          expected: expected,
    ...
    if (params.capture) {
      ...
      if (params.capture.regexp && params.capture.group) {
        capturedVal = extractor(doc, expr, params.capture.group);
      } else if (params.capture.selector) {
        capturedVal = extractor(doc, expr, params.capture);
      } else {
        capturedVal = extractor(doc, expr);
      }
      debugCapture('capture: %s = %s', params.capture.as, capturedVal);
      result.captures[params.capture.as] = capturedVal;

      if (params.capture.transform) {
        let capturedVal2 = engineUtil.evil(
          result.captures,
          params.capture.transform);

        debugCapture('transform: %s = %s', params.capture.as, result.captures[params.capture.as]);
        result.captures[params.capture.as] = capturedVal2;
      }
    }
...
function getParserAndExtractor(params, response) {
  if (params.capture || params.match) {
    let parser;
    let extractor;
    ...
    return { parser: parser, extractor: extractor };
  } else {
    return { parser: dummyParser, extractor: dummyExtractor };
  }
}
