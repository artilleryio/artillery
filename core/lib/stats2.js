...
Stats.prototype.addAssertion = function (assertionResult) {

  if (!this._assertions[assertionResult.name]) {
    this._assertions[assertionResult.name] = {}
  }

  assertionResult.results.forEach(ar => {
    const assertionName = `${ar.type} ${ar.expected}`;
    if (!this._assertions[assertionResult.name][assertionName]) {
      this._assertions[assertionResult.name][assertionName] = {success: 0, fail: 0, failureResponses: {}}
    }
    if (ar.ok) {
      this._assertions[assertionResult.name][assertionName].success++;
    } else {
      this._assertions[assertionResult.name][assertionName].fail++;
      if (!this._assertions[assertionResult.name][assertionName].failureResponses[ar.got]) {
        this._assertions[assertionResult.name][assertionName].failureResponses[ar.got] = 0
      }
      this._assertions[assertionResult.name][assertionName].failureResponses[ar.got]++;
    }
  });
  return this;
};
...
Stats.prototype.report = function () {
  ...
  result.assertions = this._assertions;
